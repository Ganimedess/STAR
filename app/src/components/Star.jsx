...
const Star = ({size = 50, color = '#fff', top = 0, left = 0}) => {
    const STROKE = 50
    const stroke = 4 + Math.floor(size/STROKE)

  return (
    <svg version="1.1" width={size} height={size} xmlns="http://www.w3.org/2000/svg" style={{transformOrigin: 'center', transform: `rotate(${Math.random()}turn)`, position: 'fixed', top: `${top}%`, left: `${left}%`}}>
