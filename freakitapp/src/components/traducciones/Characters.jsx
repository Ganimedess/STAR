...
const Characters = () => {
  ...
  return (
    ...
    <section id="personajes" className="mt-24 max-w-[1200px] m-auto max-lg:px-3">
        <h1 className='uppercase font-bold text-2xl mb-7'>Anime characters</h1>
        <ul className="grid grid-cols-3 md:grid-cols-6 gap-x-5 gap-y-7">
        {
          personalidades.map(item =>(
            item.personajes.map(element => (
              <li className=''>
                <img className='rounded-[20px]' loading="lazy" src={`/fotosPersonajes/${element.url}.jpg`} alt={`retrato ${element.nombre}`} />
